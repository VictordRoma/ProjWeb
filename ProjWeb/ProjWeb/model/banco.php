...
class Banco{
    ...
    public function updateAgendamentos($id,$email,$senha,$endereco,$bairro,$cep, $cidade, $estado){
        $stmt = $this->mysqli->query("UPDATE agendamentos SET `email` = '" . $email . "', `endereco` = '". $endereco ."', `senha` = '". $senha .  "', `bairro` = '" . $bairro . "', `cep` = '". $cep ."', `cidade` = '" . $cidade . "', `estado` = '" . $estado . "' WHERE `agendamentos`.`id` = " . $id . ";");
        if( $stmt > 0){
            return true;
        }else{
            return false;
